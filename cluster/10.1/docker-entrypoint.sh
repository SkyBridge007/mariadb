	...
	if [ ! -d "$DATADIR/mysql" ]; then
		...
		if [ "$SLAVE" = 1 ]; then
            ...
            if [ -z "$MYSQL_MASTER_PORT" ] ; then
                echo >&2 'error:  missing MYSQL_MASTER_PORT'
                echo >&2 '  Did you forget to add -e MYSQL_MASTER_PORT=...?'
                exit 1
            fi
         
            if [ -z "$MYSQL_MASTER_HOST" ] ; then
                echo >&2 'error:  missing MYSQL_MASTER_HOST'
                echo >&2 '  Did you forget to add -e MYSQL_MASTER_HOST=...?'
                exit 1
            fi
         
            if [ -n "$DATABASE_FILE" ]; then
                if [ ! -f "$DATADIR/$DATABASE_FILE" ]; then
                    echo >&2 'error: missing DATABASE_FILE'
                    echo >&2 '  $DATABASE_FILE must be a sql file in $DATADIR directory!'
                    exit 1
                fi
                echo "source $DATA_DIR/${DATABASE_FILE};"  | "${mysql[@]}"
            fi
         
            echo "CHANGE MASTER TO MASTER_HOST='${MYSQL_MASTER_HOST}',MASTER_PORT=${MYSQL_MASTER_PORT},MASTER_USER='${MYSQL_REPLICATION_USER}',MASTER_PASSWORD='${MYSQL_REPLICATION_PASSWORD}',MASTER_LOG_FILE='${MASTER_LOG_FILE}',MASTER_LOG_POS= ${MASTER_LOG_POS};"  | "${mysql[@]}"
